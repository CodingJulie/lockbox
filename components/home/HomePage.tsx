...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    if (key) {
      verifyVaultKey(key).then((result) => {
        if (result.ok) {
          storeVaultKey(key, { role: result.role, permissions: result.permissions });
        }
        setHasVault(result.ok);
        setChecking(false);
      });
    } else {
      setChecking(false);
  ...
  const handleCreateVault = async () => {
    ...
    try {
      ...
      if (err instanceof CreateVaultError && err.reason === "rate_limited") {
        toast.error(t("toast.createRateLimited"));
      } else {
